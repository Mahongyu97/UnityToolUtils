...
public static class KEngineExtensions
{
    ...
    public static Vector2 ToVector2(this Vector3 vec, bool xz = false)
    {
        if (xz)
    ...
    public static double DistanceXZ(this Vector3 origin, Vector3 dest)
    {
        float dx = origin.x - dest.x;
        float dz = origin.z - dest.z;
        return Math.Sqrt((dx * dx) + (dz * dz));
    }

    public static double MagnitudeXZ(this Vector3 vec)
    {
        float dx = vec.x;
        float dz = vec.z;
        return Math.Sqrt((dx * dx) + (dz * dz));
    ...
    public static T Get<T>(this object[] openArgs, int offset, bool isLog = true)
    {
        ...
        if ((openArgs.Length - 1) >= offset)
        {
            ...
            else
            {
                ...
                catch (Exception)
                {
                    ...
                    {
                        Debug.LogErrorFormat("[Error get from object[],  '{0}' change to type {1}", arrElement, typeof(T));
                        ret = default(T);
                    }
                }
            }
        ...
        else
        {
            ret = default(T);
            if (isLog)
                Debug.LogErrorFormat("[GetArg] {0} args - offset: {1}", openArgs, offset);
        }
